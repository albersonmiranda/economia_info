...
```{r receitas da musica, fig.cap = "Receita global da indústria da música em US\\$ bi \\citep{commons}", out.width = '80%'}
...
```{r receitas de filmes, fig.cap = "Ingressos vendidos e receita do conema nos Estados Unidos em US\\$ \\citep{filmes}", out.width = '80%'}
...
```{r receitas de games, fig.cap = "Proporção das vendas de jogos eletrônicos por mídia \\citep{games}", out.width = '80%'}
...
```{r oligo musica, fig.cap = "Quantidade de artistas diferentes compondo a Billboard 100 por ano \\citep{experimentalhistory}", out.width = '80%'}
...
```{r oligo filmes, fig.cap = "Filmes no \\textit{top} 20 de \\textit{box office} que são prequelas, sequências, \\textit{spinoffs}, \\textit{remakes}, \\textit{reboots} ou universo cinemático expandido \\citep{experimentalhistory}", out.width = '80%'}
knitr::include_graphics("img/oligo-filmes.png")
```

```{r oligo tv, fig.cap = "Programas de TV dentre os 30 de maior audiência que são \\textit{spinoffs}ou são exibidos em mais de um dia na semana \\citep{experimentalhistory}", out.width = '80%'}
...
```{r oligo livros, fig.cap = "Proporção de livros entre os mais 10 vendidos no ano por autores com mais de um livro no \\textit{top} 10 \\citep{experimentalhistory}", out.width = '80%'}
